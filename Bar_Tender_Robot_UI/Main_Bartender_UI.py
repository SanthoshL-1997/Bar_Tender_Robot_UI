# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'Bartender_UI_PYQT.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.

from PyQt5 import QtCore, QtGui, QtWidgets, QtTest

class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(800, 600)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.Glass_Position = QtWidgets.QFrame(self.centralwidget)
        self.Glass_Position.setGeometry(QtCore.QRect(330, 90, 391, 41))
        self.Glass_Position.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.Glass_Position.setFrameShadow(QtWidgets.QFrame.Raised)
        self.Glass_Position.setObjectName("Glass_Position")
        self.label_8 = QtWidgets.QLabel(self.Glass_Position)
        self.label_8.setGeometry(QtCore.QRect(10, 0, 371, 41))
        self.label_8.setObjectName("label_8")
        self.MixWith = QtWidgets.QFrame(self.centralwidget)
        self.MixWith.setGeometry(QtCore.QRect(570, 160, 151, 261))
        self.MixWith.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.MixWith.setFrameShadow(QtWidgets.QFrame.Raised)
        self.MixWith.setObjectName("MixWith")
        self.label_4 = QtWidgets.QLabel(self.MixWith)
        self.label_4.setGeometry(QtCore.QRect(9, 20, 131, 20))
        self.label_4.setObjectName("label_4")
        self.pushButton_9 = QtWidgets.QPushButton(self.MixWith)
        self.pushButton_9.setGeometry(QtCore.QRect(30, 70, 91, 31))
        self.pushButton_9.setObjectName("pushButton_9")
        self.pushButton_14 = QtWidgets.QPushButton(self.MixWith)
        self.pushButton_14.setGeometry(QtCore.QRect(30, 120, 91, 31))
        self.pushButton_14.setObjectName("pushButton_14")
        self.pushButton_18 = QtWidgets.QPushButton(self.MixWith)
        self.pushButton_18.setGeometry(QtCore.QRect(30, 170, 91, 31))
        self.pushButton_18.setObjectName("pushButton_18")
        self.No_ne = QtWidgets.QPushButton(self.MixWith)
        self.No_ne.setGeometry(QtCore.QRect(30, 220, 91, 31))
        self.No_ne.setObjectName("No_ne")
        self.ID_scan_Result = QtWidgets.QFrame(self.centralwidget)
        self.ID_scan_Result.setGeometry(QtCore.QRect(330, 30, 391, 51))
        self.ID_scan_Result.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.ID_scan_Result.setFrameShadow(QtWidgets.QFrame.Raised)
        self.ID_scan_Result.setObjectName("ID_scan_Result")
        self.label_6 = QtWidgets.QLabel(self.ID_scan_Result)
        self.label_6.setGeometry(QtCore.QRect(10, 0, 371, 51))
        self.label_6.setObjectName("label_6")
        self.ID_scan_Result_2 = QtWidgets.QFrame(self.ID_scan_Result)
        self.ID_scan_Result_2.setGeometry(QtCore.QRect(0, 50, 491, 51))
        self.ID_scan_Result_2.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.ID_scan_Result_2.setFrameShadow(QtWidgets.QFrame.Raised)
        self.ID_scan_Result_2.setObjectName("ID_scan_Result_2")
        self.NonAlcohol_2 = QtWidgets.QFrame(self.centralwidget)
        self.NonAlcohol_2.setGeometry(QtCore.QRect(440, 160, 120, 161))
        self.NonAlcohol_2.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.NonAlcohol_2.setFrameShadow(QtWidgets.QFrame.Raised)
        self.NonAlcohol_2.setObjectName("NonAlcohol_2")
        self.pushButton_16 = QtWidgets.QPushButton(self.NonAlcohol_2)
        self.pushButton_16.setGeometry(QtCore.QRect(20, 120, 91, 31))
        self.pushButton_16.setObjectName("pushButton_16")
        self.label_3 = QtWidgets.QLabel(self.NonAlcohol_2)
        self.label_3.setGeometry(QtCore.QRect(0, 10, 121, 21))
        self.label_3.setObjectName("label_3")
        self.pushButton_8 = QtWidgets.QPushButton(self.NonAlcohol_2)
        self.pushButton_8.setGeometry(QtCore.QRect(20, 70, 91, 31))
        self.pushButton_8.setObjectName("pushButton_8")
        self.AlcoholQn_3 = QtWidgets.QLabel(self.NonAlcohol_2)
        self.AlcoholQn_3.setGeometry(QtCore.QRect(0, 30, 121, 20))
        self.AlcoholQn_3.setObjectName("AlcoholQn_3")
        self.Alcoholic_Drink = QtWidgets.QFrame(self.centralwidget)
        self.Alcoholic_Drink.setGeometry(QtCore.QRect(310, 160, 120, 161))
        self.Alcoholic_Drink.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.Alcoholic_Drink.setFrameShadow(QtWidgets.QFrame.Raised)
        self.Alcoholic_Drink.setObjectName("Alcoholic_Drink")
        self.Wine = QtWidgets.QPushButton(self.Alcoholic_Drink)
        self.Wine.setGeometry(QtCore.QRect(10, 120, 91, 31))
        self.Wine.setObjectName("Wine")
        self.Beer = QtWidgets.QPushButton(self.Alcoholic_Drink)
        self.Beer.setGeometry(QtCore.QRect(10, 70, 91, 31))
        self.Beer.setObjectName("Beer")
        self.AlcoholQn = QtWidgets.QLabel(self.Alcoholic_Drink)
        self.AlcoholQn.setGeometry(QtCore.QRect(0, 10, 121, 21))
        self.AlcoholQn.setObjectName("AlcoholQn")
        self.AlcoholQn_2 = QtWidgets.QLabel(self.Alcoholic_Drink)
        self.AlcoholQn_2.setGeometry(QtCore.QRect(0, 30, 121, 20))
        self.AlcoholQn_2.setObjectName("AlcoholQn_2")
        self.Result = QtWidgets.QFrame(self.centralwidget)
        self.Result.setGeometry(QtCore.QRect(90, 390, 481, 51))
        self.Result.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.Result.setFrameShadow(QtWidgets.QFrame.Raised)
        self.Result.setObjectName("Result")
        self.label_7 = QtWidgets.QLabel(self.Result)
        self.label_7.setGeometry(QtCore.QRect(10, 0, 461, 51))
        self.label_7.setObjectName("label_7")
        self.DrinkType = QtWidgets.QFrame(self.centralwidget)
        self.DrinkType.setGeometry(QtCore.QRect(90, 160, 211, 161))
        self.DrinkType.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.DrinkType.setFrameShadow(QtWidgets.QFrame.Raised)
        self.DrinkType.setObjectName("DrinkType")
        self.Drink_Label = QtWidgets.QLabel(self.DrinkType)
        self.Drink_Label.setGeometry(QtCore.QRect(0, 10, 211, 21))
        self.Drink_Label.setObjectName("Drink_Label")
        self.NonAlcohol = QtWidgets.QPushButton(self.DrinkType)
        self.NonAlcohol.setGeometry(QtCore.QRect(10, 120, 191, 31))
        self.NonAlcohol.setObjectName("NonAlcohol")
        self.Alcohol = QtWidgets.QPushButton(self.DrinkType)
        self.Alcohol.setGeometry(QtCore.QRect(10, 70, 191, 31))
        self.Alcohol.setObjectName("Alcohol")
        self.AlcoholQn_4 = QtWidgets.QLabel(self.DrinkType)
        self.AlcoholQn_4.setGeometry(QtCore.QRect(0, 30, 211, 21))
        self.AlcoholQn_4.setObjectName("AlcoholQn_4")
        self.pushButton = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton.setGeometry(QtCore.QRect(90, 30, 231, 101))
        self.pushButton.setObjectName("pushButton")
        self.NA_Extended = QtWidgets.QFrame(self.centralwidget)
        self.NA_Extended.setGeometry(QtCore.QRect(440, 320, 121, 71))
        self.NA_Extended.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.NA_Extended.setFrameShadow(QtWidgets.QFrame.Raised)
        self.NA_Extended.setObjectName("NA_Extended")
        self.No_ne_Extended = QtWidgets.QPushButton(self.NA_Extended)
        self.No_ne_Extended.setGeometry(QtCore.QRect(20, 10, 91, 31))
        self.No_ne_Extended.setObjectName("No_ne_Extended")
        self.frame = QtWidgets.QFrame(self.centralwidget)
        self.frame.setGeometry(QtCore.QRect(540, 480, 181, 31))
        self.frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame.setObjectName("frame")
        self.No_ne_2 = QtWidgets.QPushButton(self.frame)
        self.No_ne_2.setGeometry(QtCore.QRect(60, 0, 91, 31))
        self.No_ne_2.setObjectName("No_ne_2")
        MainWindow.setCentralWidget(self.centralwidget)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)
        self.actionOpen = QtWidgets.QAction(MainWindow)
        self.actionOpen.setObjectName("actionOpen")
        self.actionClose = QtWidgets.QAction(MainWindow)
        self.actionClose.setObjectName("actionClose")

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.label_8.setText(_translate("MainWindow", "   "))
        self.label_4.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">Mix with?</span></p></body></html>"))
        self.pushButton_9.setText(_translate("MainWindow", "Coke"))
        self.pushButton_14.setText(_translate("MainWindow", "Water"))
        self.pushButton_18.setText(_translate("MainWindow", "Ice"))
        self.No_ne.setText(_translate("MainWindow", "None"))
        self.label_6.setText(_translate("MainWindow", "    "))
        self.pushButton_16.setText(_translate("MainWindow", "Water"))
        self.label_3.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">Non Alcoholic</span></p></body></html>"))
        self.pushButton_8.setText(_translate("MainWindow", "Coke"))
        self.AlcoholQn_3.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">Drink</span></p></body></html>"))
        self.Wine.setText(_translate("MainWindow", "Wine"))
        self.Beer.setText(_translate("MainWindow", "Beer"))
        self.AlcoholQn.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">Alcoholic</span></p></body></html>"))
        self.AlcoholQn_2.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">Drink</span></p></body></html>"))
        self.label_7.setText(_translate("MainWindow", "Hey user, Enjoy your drink"))
        self.Drink_Label.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">What drink would you</span></p></body></html>"))
        self.NonAlcohol.setText(_translate("MainWindow", "Non alcoholic drink"))
        self.Alcohol.setText(_translate("MainWindow", "Alcoholic drink"))
        self.AlcoholQn_4.setText(_translate("MainWindow", "<html><head/><body><p align=\"center\"><span style=\" font-weight:600;\">love to have?</span></p></body></html>"))
        self.pushButton.setText(_translate("MainWindow", "Insert Your ID Card to scan"))
        self.No_ne_Extended.setText(_translate("MainWindow", "None"))
        self.No_ne_2.setText(_translate("MainWindow", "Reset"))
        self.actionOpen.setText(_translate("MainWindow", "Open"))
        self.actionClose.setText(_translate("MainWindow", "Close"))
#Hiding Frames
        self.ID_scan_Result.hide() #Hey user, you are eligible / non eligible to drink
        self.Glass_Position.hide() #Place Your glass
        self.DrinkType.hide() #What drink would you love to have?
        self.Alcoholic_Drink.hide() #Alcoholic Drink
        self.NonAlcohol_2.hide() #Non Alcoholic Drink
        self.MixWith.hide() #Mix with?
        self.Result.hide() #Hey user, Enjoy your drink
        self.NA_Extended.hide() # Hidden from NonAlcohol Extended

# adding action to each of the button
        self.pushButton.clicked.connect(self.action_pushButton)
        self.Alcohol.clicked.connect(self.action_Alcohol)
        self.NonAlcohol.clicked.connect(self.action_NonAlcohol)
        self.Beer.clicked.connect(self.action_Beer)
        self.Wine.clicked.connect(self.action_Wine)
        self.pushButton_8.clicked.connect(self.action_Coke)
        self.pushButton_16.clicked.connect(self.action_Water)
        self.pushButton_9.clicked.connect(self.action_MixCoke)
        self.pushButton_14.clicked.connect(self.action_MixWater)
        self.pushButton_18.clicked.connect(self.action_MixIce)
        self.No_ne_2.clicked.connect(self.action_reset)
        self.No_ne.clicked.connect(self.action_None)
        self.No_ne_Extended.clicked.connect(self.action_None_extended)

    def action_pushButton(self):
        print("Pushed ID Card Detection")
        self.ID_scan_Result.show()  # Hey user, you are eligible / non eligible to drink
        self.label_6.setText("Hello, scanning your card closely....")
        QtTest.QTest.qWait(1000)
        self.label_6.setText("Eligibility Check....")
        QtTest.QTest.qWait(1000)
        age = 23 #This age variable to be replaced by adding "ID Card detected value"
        if (age>=21):
            self.pushButton.setStyleSheet("background-color : green")
            self.label_6.setText("You are eligible to have an alcoholic drink")
            QtTest.QTest.qWait(1000)
            self.Glass_Position.show()
            self.label_8.setText("Place your glass")
            QtTest.QTest.qWait(1000)
            self.DrinkType.show()
        else:
            self.pushButton.setStyleSheet("background-color : red")
            self.label_6.setText("You are not eligible to have an alcoholic drink")
            QtTest.QTest.qWait(1000)
            self.NonAlcohol_2.show()
            self.NA_Extended.show()

    def action_Alcohol(self):
        print("Pushed Alcohol")
        self.Alcohol.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.Alcoholic_Drink.show()

    def action_NonAlcohol(self):
        print("Pushed Non Alcohol")
        self.NonAlcohol.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.NonAlcohol_2.show()

    def action_Beer(self):
        print("Pushed Beer")
        self.Beer.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.MixWith.show()

    def action_Wine(self):
        print("Pushed Wine")
        self.Wine.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.MixWith.show()

    def action_Coke(self):
        print("Pushed Coke")
        self.pushButton_8.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.Result.show()

    def action_Water(self):
        print("Pushed Water")
        self.pushButton_16.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.Result.show()

    def action_MixCoke(self):
        print("Pushed MixCoke")
        self.pushButton_9.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.Result.show()

    def action_MixWater(self):
        print("Pushed MixWater")
        self.pushButton_14.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.Result.show()

    def action_MixIce(self):
        print("Pushed MixIce")
        self.pushButton_18.setStyleSheet("background-color : green")
        QtTest.QTest.qWait(1000)
        self.Result.show()

    def action_None(self):
        print("None")
        self.No_ne.setStyleSheet("background-color : green")

    def action_None_extended(self):
        print("None_extended")
        self.No_ne_Extended.setStyleSheet("background-color : green")

    def action_reset(self):
        print("reset")
        QtTest.QTest.qWait(1000)

        self.ID_scan_Result.hide() #Hey user, you are eligible / non eligible to drink
        self.Glass_Position.hide() #Place Your glass
        self.DrinkType.hide() #What drink would you love to have?
        self.Alcoholic_Drink.hide() #Alcoholic Drink
        self.NonAlcohol_2.hide() #Non Alcoholic Drink
        self.MixWith.hide() #Mix with?
        self.Result.hide() #Hey user, Enjoy your drink
        self.NA_Extended.hide() # Hidden from NonAlcohol Extended
        self.pushButton.setStyleSheet("background-color : none")
        self.Alcohol.setStyleSheet("background-color : none")
        self.NonAlcohol.setStyleSheet("background-color : none")
        self.Beer.setStyleSheet("background-color : none")
        self.Wine.setStyleSheet("background-color : none")
        self.pushButton_8.setStyleSheet("background-color : none")
        self.pushButton_16.setStyleSheet("background-color : none")
        self.pushButton_9.setStyleSheet("background-color : none")
        self.pushButton_14.setStyleSheet("background-color : none")
        self.pushButton_18.setStyleSheet("background-color : none")
        self.No_ne.setStyleSheet("background-color : none")
        self.No_ne_Extended.setStyleSheet("background-color : none")


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
